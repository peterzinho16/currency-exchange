...
export class ConverterComponent implements OnInit {
    ...
    exchangeRatesLocal(): void {
        this.fromId = this.filterSelectedValue(FormNames.FromControl).id;
        this.toId = this.filterSelectedValue(FormNames.ToControl).id;

        //Calculating the row reference to the two currencies involved in the exchange
        const exRates = JSON.parse(StorageService.getItem("EX_RATES"));
        var finalists = exRates.filter(e => e.sourceId === this.fromId && e.destinyId === this.toId).map(obj => {
            obj.creationDate = new Date(obj.creationDate).getTime();
            return obj;
        })

        const bingo = finalists.map(a=> a.creationDate).reduce((a, b) => Math.max(a, b));
        var latest = finalists.find(item => item.creationDate === bingo)
        //End calculating--

        const rateInfo =  latest;
        console.log('Calculating...')
        this.fromRate = rateInfo.sourceUnit;
        console.log(exRates)
        this.fromCurrency = this.filterSelectedValue(FormNames.FromControl).currency;
        this.toRate = rateInfo.destinyRate;
        this.toCurrency = this.filterSelectedValue(FormNames.ToControl).currency;
    ...
    mapResponseDataLocal(responseData: CurrencyResponse[]): MappedCurrencyRateObject[] {
        return responseData.map((item: CurrencyResponse): MappedCurrencyRateObject =>
                    { return {
                        id: item.id,
                        currency: item.code,
                        rate: 1
    ...
    getRates(): void {
        if (!this.currencyExchangeService.exchangeRates) {
            if(StorageService.getItem('OWN') === 'on') {
                ...
                this.apiRequestService.getExchangeRatesLocal().subscribe(
                    (exchangeRate: CurrencyResponse[]): void => {
                        console.log(exchangeRate);

                        this.currencyExchangeService.exchangeRates = this.mapResponseDataLocal(exchangeRate);
                ...
                this.apiRequestService.getExchangeRatesToCompute().subscribe(
                    (exchangeRate: ExchangeRateResponse[]): void => {
                        const enabled = exchangeRate.filter(e => e.enabled)
                        console.log(enabled);
                        StorageService.setItem('EX_RATES', JSON.stringify(enabled));
                    },
                    (error): void => {
                        this.alertService.error(`Error: ${error.message}`);
                    },
